...
**Block Floating Point (BFP) の文脈と現状の前提**: 動機は「FPGA側でのスケーリングが画像によって大きく変動する」現象の検証。BFP はブロック単位で shift を決める手法で、純固定小数点（全データで同じ shift = 最悪ケースで決まる）と浮動小数点（サンプルごとに指数 = 高コスト）の中間。今の Route B は **per-stage の理想 BFP**（auto-scale で peak を見てから決める = シーン全体を見終わってから最適 shift を選ぶ上限性能）になっている。実 FPGA より楽観的な評価。将来の拡張オプション:
1. **固定 shift モード**: auto-scale を切って `mode='fixed'` で Stage B/C に固定 shift。画像で saturate / underflow がどう変動するか直接観測
2. **per-pulse BFP モード**: `mode='per_pulse'` で pulse(row) ごとに shift（SAR の SDF FFT で標準）
3. **two-pass キャリブレーション**: 1パス目で peak 測定 → shift 決定 → 2パス目で実処理。実 FPGA の「キャリブ → 本番」モデル
