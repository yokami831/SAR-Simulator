...
Step 1 — 信号レベル正規化（AGC 相当）:
```
sig_peak_target = 10^(signal_peak_dbfs / 20) × ADC_FS_V
gain = sig_peak_target / current_peak
s_raw *= gain
```
...
**重要な前提（実 FPGA との差）**:
- n25 は **AGC を内蔵**しており signal_peak_dbfs に常に正規化する。σ°_max を変えても ADC 入力ピークは変わらない。リアル AGC のない固定ゲイン受信機の場合は別途実装が必要
- σ°_max は **Stage A 内部の BRAM スケーリング shift bit 量を変える**（fir_coefficients の peak が変わるため）が、最終 ADC ピークは AGC で吸収される
- 「scene による絶対レベル変動」を ADC ピークで観測したい場合は AGC を切る別モード（未実装）が必要
