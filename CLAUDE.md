...
- Stage A 入力時間領域: `Q_IN_W=16, Q_IN_F=15` (Q1.15)
- Stage B FFT 後周波数領域 + 乗算後: `Q_FFT_W=18, Q_FFT_F=10` (整数部 8bit、log2(Nr) ~14 のビット成長を吸収)
- Stage C IFFT 後時間領域: `Q_OUT_W=16, Q_OUT_F=15` (Q1.15)
- 各ステージは auto-scale (peak → Q full-scale × 0.95) で量子化し、SNR を per-stage で表示

**依存**: `fxpmath>=0.4.9` を `requirements.txt` に追加済み。新環境では `pip install fxpmath` を忘れずに。
