...
> **ポート番号について:** 以下の例で使う `18731` は優先デフォルトであり、ポートが使用中の場合は別の番号にシフトする。実際に使われているポートはプロジェクトルートの `.hiyocanvas-runtime.json`（`server_port` / `server_url`）を読むか、`/api/health` で確認すること。
...
### Stop (graceful)
```bash
python .claude/skills/hiyocanvas/scripts/canvas_api.py shutdown
```
...
### Check health
```bash
python .claude/skills/hiyocanvas/scripts/canvas_api.py status
curl http://127.0.0.1:18731/api/health
```
ポート 18731 はデフォルト。シフトしている場合は `.hiyocanvas-runtime.json` の `server_url` を使うこと。
